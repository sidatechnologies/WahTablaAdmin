...
import { updateExamAttempt } from '@/lib/actions/update-examp-attempts';
...
export const useUpdateExamAttempt = (options?: UseUpdateExamAttemptOptions) => {
  ...
  return useMutation<UpdateExamAttemptResult, Error, UpdateExamAttemptRequest>({
    mutationFn: async (request: UpdateExamAttemptRequest) => {
      return await updateExamAttempt(request);
    },
    onSuccess: (result) => {
